...
from utils import NMS, Re2Sq, MRe2Sq
...
class Detect():
    ...
    def detect(self, image):
        ...
        if rnet_box.shape[0] == 0:
            return np.array([]), pnet_time, rnet_time, 0
        ...
        if onet_box.shape[0] == 0:
            return np.array([]), pnet_time, rnet_time, onet_time


        return onet_box, pnet_time, rnet_time, onet_time
    ...
    def _pnet(self, image, stride, side_len):
        ...
        while len > 30:
            ...
            idx = np.stack(np.where(cond > 0.4), axis=1)
            i = idx[:, 0]
            j = idx[:, 1]
            cond_ = cond[i, j]
    ...
    def _rnet(self, image, pnet_box, side_len):
        ...
        idx = np.stack(np.where(cond > 0.55), axis=1)
        i = idx[:, 0]
        j = idx[:, 1]
        ...
        return NMS(np.array(rnetboxes), 0.4, True)
    ...
    def _onet(self, image, rnet_box, side_len):
        ...
        idx = np.stack(np.where(cond > 0.95), axis=1)
        ...
        onet_box_ = NMS(onetboxes, 0.2, True)


        return MRe2Sq(onet_box_)
class Genimage:
    ...
    def gen(self, i_path):
        ...
        # image_path1 = '/home/ray/datasets/Mtcnn/test/pic/{}.jpg'.format(pic)
        # i_path = '/home/ray/datasets/centerloss/data/centerloss1'
        # s_path = '/home/ray/datasets/centerloss/data/centerloss'
        for _, dirs, _ in os.walk(i_path):
            for dir in dirs:

                for _, _, filenames in os.walk(os.path.join(i_path, dir)):

                    for filename in filenames:


                        image_path = os.path.join(i_path, dir, '{}'.format(filename))
                        # save_path = os.path.join(s_path, dir)
                        ...
                        if boxes.shape[0] != 0:
                            ...
                            for box in boxes:
                                ...
                                if not os.path.exists(save_path):
                                    os.mkdir(save_path)
                                image.save(os.path.join(save_path, '{}'.format(filename)))
